...
4. **Landing-scoped focus ring** (`src/app/(marketing)/layout.tsx`, `src/app/globals.css`) — the shared `.light` block sets `--ring: oklch(0.708 0 0)`, roughly 1.5:1 grey on white. The marketing wrapper now carries a `marketing` class alongside `light`, and `globals.css` adds a `.light.marketing { --ring: var(--primary); }` compound-selector rule right after the `.light` block, so focus is clearly visible in the landing's teal accent. A compound selector (0-2-0 specificity) beats the unlayered `.light` block (0-1-0); a Tailwind utility on the same element could not, because `.light` is unlayered CSS. Scoped to `.light.marketing` rather than `.light` itself so the app's own light theme (next-themes can put `.light` on `<html>`) keeps its neutral ring.
...
### Copy rulings made mid-flight

8. **FAQ data answer reworded to "no card numbers"** (`site.ts`) — the draft answer to "What data do you store about my clients?" said "no payment details". `FORBIDDEN_COPY` contains `"payment"`, so the never-advertises-unshipped-features test failed on a sentence that was in fact *denying* the feature. Rather than weaken the guard with an exception, the copy became "No documents, no card numbers, no accounts." — same meaning, and the blanket word ban stays blunt and cheap to enforce.
9. **Embed snippet slug derived from `SITE.name`** (`mocks/embed-snippet-mock.tsx`) — the mock's `<div id="…-widget">` and script host were hardcoded to the product name, which is the one string the spec insists must flow from config. The mock now computes `const slug = SITE.name.toLowerCase()`, so a rename cannot leave a stale name inside the illustration.
10. **Embed prose names no URL** (`embed-showcase.tsx`) — the spec sketch mentioned showing `booklo.example/book/your-handle` in the body copy. Prose that quotes a hostname we do not own reads as a real, checkable link; the paragraph says "your own handle" instead and only the snippet mock carries the illustrative `booklo.example` domain, where its fictional framing is obvious from the surrounding code.

### Final-review fix wave

11. **AA contrast for the teal accent** (`globals.css`) — measured with an oklch→sRGB→relative-luminance script, `--primary: oklch(0.62 0.12 187)` (`#009d92`) gave 3.28:1 for white button labels and 3.37:1 for the eyebrow on white: both fail AA for 16px text. `.light.marketing` now overrides `--primary: oklch(0.52 0.11 187)` (`#007c73`), which measures **4.96:1** and **5.08:1**. `--primary-foreground` stays white and the ring still tracks `var(--primary)`. The override lives in the marketing compound selector, so the app's own tokens are unchanged — this is a landing-page decision, not a design-system change.
12. **Section ids derived from config** (`site.ts`, `how-it-works.tsx`, `feature-grid.tsx`, `faq.tsx`) — the three sections carried hardcoded `id="how-it-works"` / `"features"` / `"faq"` strings that duplicated `SITE.anchors`, so renaming an anchor would have silently broken the nav links pointing at it. A new `anchorId(anchor)` helper strips the leading `#` (throwing if it is absent) and each section now uses `id={anchorId(SITE.anchors.<key>)}`. The old test ("anchors used in nav exist as section ids") only compared config against itself and could not have caught the drift; it is replaced by a real nav/anchor check, an `anchorId` rejection case, and a source-level guard that reads the three component files and asserts each contains its `anchorId(SITE.anchors.<key>)` call.
13. **All copy centralized** (`site.ts` + every section component) — the spec requires that "all copy lives in `site.ts` as typed data", but section headings, sub-lines, the product bullets, the embed paragraphs and the CTA button labels were still literals in JSX. They now live in `SECTIONS` (`how`, `features`, `product` with `points`, `embed` with `paragraphs`, `faq`) and `CTA` (`login`, `getStarted`, `getStartedFree`, `seeHow`), and the components read from them. The forbidden-copy test corpus was extended to cover every `SECTIONS` string and `CTA` label, so the guard now sees the copy it was always meant to police. Mock-internal fake data (client names, times, snippet lines) deliberately stays in the mocks — that is illustration, not marketing copy.
14. **`dark:` variants opt out under `.light`** (`globals.css`) — `@custom-variant dark (&:is(.dark *))` matched anything inside the dark `<html>`, including the light-scoped marketing tree, so shared components' `dark:` utilities (e.g. the button's ghost hover) still fired on the landing page even though its tokens were light. The variant is now `(&:is(.dark *):not(.light *))`. Verified in the compiled CSS: all 31 `:is(.dark *)` selectors carry the `:not(.light *)` guard. Nothing under `(dashboard)` renders inside a `.light` wrapper, so the app is unaffected.
15. **`.light` comment narrowed** (`globals.css`) — the block said "Keep in sync with `:root`", which promised a parity it does not have. It now states that it intentionally covers only the core tokens the marketing surface uses (no sidebar, chart or status tokens) and that anything nested inside needing those must add them.
16. **`tabIndex={-1}` on the aria-hidden scrollers** (`mocks/calendar-mock.tsx`, `mocks/embed-snippet-mock.tsx`) — both mock cards are `aria-hidden="true"`, but their scroll containers were still keyboard-focusable, which parks a tab stop on a region screen readers cannot describe. Both are now explicitly removed from the tab order.
17. **Calendar mock shows `09:00`** (`mocks/calendar-mock.tsx`) — the first hour label was blanked (`i === 0 ? "" : …`) because the shared `-translate-y-1/2` centring pushed it under the day-header row. The label is now rendered like the rest; only the first one swaps the centring translate for a small `translate-y-0.5` nudge so it hangs below the header instead of behind it. Confirmed in the final screenshot.

---

## Deferred follow-ups

Not blocking; none of these change what ships today.

- **Extract `<Section>` / `<SectionHeading>`** — the `border-t` + `mx-auto w-full max-w-6xl px-6 py-20 md:py-28` wrapper is repeated across six sections, as is the heading/sub-line pair. Worth extracting once a seventh section appears or the rhythm needs to change in one place.
- **OG image and canonical URL** — add `opengraph-image.tsx` under `(marketing)` and set `metadataBase` + a canonical URL in the layout's metadata, so shared links preview properly. Needs the production domain to be settled first.
- **Apply the `.light` wrapper to `/book/[handle]`** — the real booking page still renders dark while the landing's booking mock shows it light, so the page a visitor lands on does not match what the marketing page promised. Out of scope here (the brief excludes `src/app/book/**`).
- **`color-scheme: dark` on the `.dark` block** (pre-existing) — `.light` declares `color-scheme: light`, but `.dark` never declares its counterpart, so native controls (scrollbars, form widgets) in the app render with light-mode chrome.
- **`booking-card-mock.tsx` ternaries → `cn()`** — the mock picks class strings with inline ternaries where the rest of the codebase composes with `cn()`; a readability nit only.
