...
- A root URL carrying a pre-branch `?space=<id>` link redirects to the spaces page with the space preselected (`rootRedirect`, `lib/booking/channel-pages.ts`), the same temporary redirect `?channel=spaces` takes. §3.2 reads accordingly.
- §2's "`revalidatePath` on publish covers `/[handle]` and `/[handle]/spaces`" is withdrawn: the public routes are dynamic (they await `searchParams`), so only `/booking-page` is revalidated, as before.
- "View live page" in the builder is shown only while the page's channel is publicly reachable (`publicReachable`, the same rule as `resolveChannelPage`); an empty channel's page would 404.
- Template thumbnails never render the cross-link (`crossLink: null` in `TemplateThumb`).
- Ruling-5 consequence, deferred (§7): when a spaces-only org later enables appointments and adds a bookable service, `/<handle>` moves to an appointments page that has no document yet (`DEFAULT_PAGE`). The checklist's "Publish your page" chip un-ticks; a persistent notice on `/booking-page` and `/bookings` for that state is a follow-up, as is the stored front door.
- Deploy note: 0060 drops the 0048 RPC signatures, so migrate and promote in one window (studio writes fail between them; public pages are unaffected). Before the production migrate, run `select o.handle, s.slug from staff s join orgs o on o.id = s.org_id where s.slug in ('spaces','appointments')` — a row needs a rename migration in the same window.
...
- Slice 2: `skinDefault(anyPublished)` deliberately ignores the website embed — an embed-only org that never published a page still gets the look checkbox pre-checked; `STARTER.lookHint` under the checkbox says so, and unchecking is one click.
- Slice 2: when `createOffering` saves the space but the plan refuses its first unit or hours (`notice`), the first-space step holds — the notice shows inline with an explicit Continue — instead of advancing on a toast; the page then applies over a space the public page will not list until it has a unit.
- Slice 2: card thumbnails use `typePreview` (template sample copy + the type's widget title and cover button), so the two Venue-backed types read as themselves.
- Migration **0061** (`reserved_staff_slugs`, custom) renames any existing staff slugged `spaces` / `appointments` to `<slug>-1` — idempotent, guarded against a taken `<slug>-1`. It replaces the pre-deploy production query in §3.3/§7 (production credentials are write-only from a workstation), so the deploy needs no manual step. **H4 renumbers to 0062.**
