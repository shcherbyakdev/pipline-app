...
**Amended 2026-08-27 — two booking sections.** The `booking` section carries `channel: "all" | "appointments" | "spaces"` (optional; absent = `all`, the combined widget every stored page has — and what "Show: Appointments & spaces" writes back, so an untouched widget stays byte-equal to its published one). Page rule (Zod, and the cap in `save_booking_page_draft`, migration 0059): 1–2 booking sections; two only as `appointments` + `spaces`; at least one visible. `hidden` is a real boolean on booking sections now; `canHideSection` / `canRemoveSection` (`doc-ops.ts`) refuse the last visible widget, and the row shows the old "Required" badge in that case. Labels come from `sectionLabel(section)` — "Booking", "Book appointments", "Book spaces" (vocab `bookSection`). Splitting: the combined widget's inspector (both-channel present mode only) gets **Show** (Appointments & spaces / Appointments only / Spaces only) and **Split into two sections** (`splitBookingSection`: appointments keeps the id and title, spaces follows); the palette offers "Book appointments" / "Book spaces" while the channel has no widget (`addableEntries`; the combined widget counts as both: "Split the Booking section first."). A per-channel widget renders the same `BookingWidget` with the other channel's list emptied; DOM ids `#book` (all / appointments) and `#book-spaces` (`bookingAnchor`); Spaces cards scroll to `#book-spaces` when it exists; `bookHref` falls back to the first visible widget. A Services / Spaces cards section whose channel no visible widget books is treated as empty (`publicSections` drops it, the Publish warning names it) so cards never lead nowhere. Templates and the default page stay combined.
