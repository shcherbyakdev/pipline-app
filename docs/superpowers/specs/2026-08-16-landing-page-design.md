...
> **Palette update (2026-08-17):** the landing moved from the light/teal scheme described below to a dark forest palette modelled on deck.co — near-black ground `#0D100F`, off-white type `#F6F6F6`, cards one step up (`#161A19`), white-alpha hairlines, and a single pale-yellow accent `#FDFAC3` (with near-black text) for primary CTAs, eyebrow, step numerals and "live" calendar state. The mechanism is unchanged: a `.marketing { … }` token block in `globals.css` on the layout wrapper (`<div className="marketing …">`), `color-scheme: dark`, tokens only in components. The `.light` block stays in `globals.css` for future light surfaces (e.g. `/book`). Same day: the hero calendar got its own `--hero-accent` (mint green) so it doesn't share the CTA yellow; a CSS "grainient" hero backdrop was tried (grainient.supply dark/lime preset recreated with radial gradients + SVG-noise grain) and removed the same day at the user's call; nav + buttons were restyled after throxy.com in our palette (`marketing-button.ts`: rectangular 4px-radius CTAs, long arrow leading the label, plain-text nav links, no nav hairline); and landing typography switched to Geist / Geist Mono (scoped in the marketing layout; throxy-style medium-weight, tight-tracked headings). PP Neue Montreal (throxy's face) is commercial and was not used. Later the same day the palette was re-based on linear.app's dark tokens (ground `#08090a`, levels `#0f1011/#141516/#191a1b`, text `#f7f8f8/#8a8f98`, white-alpha hairlines) keeping the yellow CTA; the hero calendar accent became Linear's indigo gradient (`#828fff → #5e6ad2`, complement to the yellow), and the hero copy got Linear's word-by-word blur/rise reveal + staggered fade-ups (`hero-reveal.module.css`, pure CSS, reduced-motion aware, sr-only full heading for a11y).
