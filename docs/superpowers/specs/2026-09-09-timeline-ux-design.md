...
States
- [x] Empty (no spaces) state; loading: `aria-busy` + subtle progress while a window transition is pending; failed move → toast + revert; expired holds/lapsed requests never drawn.

Accessibility
- [x] Roving-tabindex grid, labelled cells, buttons for bars/chips with full aria-labels, live region for drag/window announcements, visible focus rings, contrast ≥ 4.5:1 for bar text in light and dark, reduced-motion respected, conflicts carry an icon not only colour.
