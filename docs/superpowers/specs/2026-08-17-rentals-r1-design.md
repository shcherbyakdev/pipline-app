...
2. **Named units + auto-assignment**, not pooled capacity. Every physical thing is a `units` row; a booking is pinned to exactly one unit; a "car class" is an offering with 3 unit rows. Reuses the EXCLUDE guard verbatim. Pooled capacity (`capacity: N`, count-overlap check) can be added later as an offering without units if a real customer needs it.
...
New Drizzle file `src/db/schema/rentals.ts`; one custom SQL migration `0036_rentals_security.sql` holding CHECKs, RLS, GRANTs, guards, and the RPC (the 0026 idiom: security surface in one reviewable place). Explicit GRANTs on every new table (repo convention).
...
### `unit_blackouts`
`id`, `org_id`, `unit_id` (→ units cascade), `start_date` date, `end_date` date (both org-local, **inclusive**; CHECK `end_date >= start_date`), `reason` text null (≤ 500), `created_at`. Index `(unit_id, start_date)`.

### `bookings` (additive)
- `+ rental_offering_id uuid null` → rental_offerings **restrict**
- `+ unit_id uuid null` → units **restrict**
- `service_id` becomes **nullable**
- CHECK `bookings_kind`: `(service_id is not null) <> (rental_offering_id is not null)`
- CHECK `bookings_unit_iff_rental`: `(unit_id is not null) = (rental_offering_id is not null)`
- Existing `bookings_no_overlap` is **recreated** with `where (status = 'confirmed' and unit_id is null)` — appointments only.
- New `bookings_unit_no_overlap`: `exclude using gist (unit_id with =, tstzrange(starts_at, ends_at) with &&) where (status = 'confirmed' and unit_id is not null)`.
- Index `(unit_id, starts_at)`.
...
Existing rows: all have `service_id`, `unit_id` null → both CHECKs hold; the recreated appointment guard is equivalent for them.

### RLS / grants
- `rental_offerings`, `units`, `unit_blackouts`: org-member `select/insert/update/delete` via the established `org_id in (select public.user_orgs())` policy pattern (0026); explicit GRANTs to `authenticated`. **No anon policies** — the public page reads through the admin client scoped by resolved handle (`lib/booking/public.ts` precedent, S3 decision), and writes only through the definer RPC.
...
- **`/rentals`** — offerings list. Card per offering: name, mode badge ("Nightly" / "Daily"), unit count, price label, active toggle, drag-order (reuse `services-list` idiom). Create/edit dialog (`offering-dialog.tsx`, modeled on `service-dialog.tsx`) with fields grouped: *Basics* (name, description, price label) · *Stay* (mode, start/end time via `time-options.ts`, min/max stay, turnover days) · *Booking* (notice days, window days, unit selection).
- **`/rentals/[id]`** — units for one offering: inline list (add / rename / description / active / reorder / delete) and, per unit, blackouts (add date range + optional reason, list, delete). Form-heavy page, no calendar.
...
- **Emails** — `bookingConfirmationEmail` / reminder / cancellation templates gain a rental variant of the "when" line: "{offering} · {unit} · Fri 12 Sep 15:00 → Mon 15 Sep 11:00 (Europe/Berlin)". `formatWhenLine` gets a rental branch keyed on `rental_offering_id`. ICS: single event with the real check-in/out instants (not `VALUE=DATE` all-day; the instants are correct and calendar apps render multi-day bars fine).
...
- RLS tests for `rental_offerings`, `units`, `unit_blackouts`.
