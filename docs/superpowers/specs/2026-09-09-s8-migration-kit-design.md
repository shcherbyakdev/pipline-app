...
Studio-facing import UI; dates-mode rows; people/extras/equipment columns; importing the old price; confirmation or manage-link mails for imported bookings (Booklo sends none — but a connected Google Calendar with *Invite guests* on will invite each client, hence the checklist's "connect the calendar after the import" step); a unit column to pin a specific unit; export of Booklo bookings back to CSV; an org-id assertion before `mark_booking_paid` (the service-role gate on that RPC has no org anchor; today its only service-role caller sits behind `requireInternal()`). Pre-existing console warning ("Encountered a script tag while rendering React component") appears on every `/utils` page — not S8's.
