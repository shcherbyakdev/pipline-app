...
## 8. Web Push keys (VAPID)

Push notifications (`/notifications`, spec 2026-09-05) sign every send with
one VAPID key pair. Three Vercel environment variables, Production scope:

```bash
npx web-push generate-vapid-keys      # prints a public and a private key
vercel env add VAPID_PUBLIC_KEY production
vercel env add VAPID_PRIVATE_KEY production
vercel env add VAPID_SUBJECT production   # mailto:hello@booklo.co
```
...
| `GOOGLE_SITE_VERIFICATION` | optional; Search Console token so Google may push calendar changes | wizard stage 6; absent = poll every 15 min |
...
**Google → Booklo (v2).** Two per-connection switches let a delete in
Google cancel the booking and a move reschedule it (Calendly's semantics).
Google pushes those changes to `/api/google/webhook` through a channel
Booklo opens per connection (7-day cap, renewed by the tick when under a
day left) — but only on a **verified domain**: `GOOGLE_SITE_VERIFICATION`
(Search Console HTML-tag token, rendered as the site's verification meta
tag) plus the domain added under Cloud console → Domain verification. The
wizard's stage 6 walks it. Without it, or locally, the tick polls every
connection with a switch on every 15 minutes (`inbound` in the drain
answer). A refused move (slot taken, outside hours) is put back in Google
and shown on the /integrations row; the connecting user gets a push.
