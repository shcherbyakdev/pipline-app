...
import { billingErrorKey } from "@/features/billing/schema";
...
export default async function BillingPage({ searchParams }: PageProps<"/billing">) {
  ...
  // Pinned locale + UTC (current-plan.tsx idiom): the date must not depend
  // on the runtime's locale or zone.
  const formatDate = (iso: string) =>
    new Intl.DateTimeFormat(INTL_LOCALES[locale as Locale], { day: "2-digit", month: "short", year: "numeric", timeZone: "UTC" })
      .format(new Date(iso));
    ...
    <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 p-6">
      ...
      {comped ? (
        <p className="text-muted-foreground text-sm">
          {override.expiresAt ? t("complimentaryUntil", { date: formatDate(override.expiresAt) }) : t("complimentary")}
        </p>
      ) : (
        <PlanPicker
