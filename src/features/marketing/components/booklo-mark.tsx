...
/* The Booklo mark (2026-09-09): one soft blob, the periwinkle gradient
   ...
   Sized by `className` (defaults to the parent's font-size); `ink` fills
   it with the current text colour instead. */
...
export function BookloMark({ className, ink = false }: { className?: string; ink?: boolean }) {
  ...
  return (
    <svg viewBox="0 0 200 200" aria-hidden="true" className={cn("size-[1em] shrink-0", className)}>
      ...
      <path fill={ink ? "currentColor" : `url(#${id})`} d={MARK_PATH} />
    </svg>
  );
}
...
/* Mark beside wordmark, on one baseline. `ink` draws the mark in the
   text colour (the nav's dark logo) instead of the gradient. */
export function BookloLogo({ className, ink = false }: { className?: string; ink?: boolean }) {
  return (
    <span className={cn("inline-flex items-center gap-[0.32em]", className)}>
      <BookloMark className="size-[1.1em]" ink={ink} />
      <BookloWordmark />
    </span>
  );
}
