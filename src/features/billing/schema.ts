...
/* A plain `<form action={serverAction}>` discards whatever the action returns,
   ...
   this page wants one). The copy lives in `errors.billing.*` (i18n Wave 3);
   this maps the URL code to its key. */
...
export type BillingErrorKey = (typeof BILLING_ERRORS)[BillingErrorCode];

/** The `errors.billing.*` key for `?error=`; null for a missing or unknown code. */
export function billingErrorKey(code: string | string[] | undefined): BillingErrorKey | null {
  return typeof code === "string" && code in BILLING_ERRORS ? BILLING_ERRORS[code as BillingErrorCode] : null;
}
