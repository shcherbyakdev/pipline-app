...
/* The one gate in front of every `/dev/billing` page and every dev server
   ...
   - BILLING_ENABLED — while the flag is off there is no billing surface at
     all, so the emulator has nothing to emulate;
...
export async function requireDevBilling(orgParam?: string | null): Promise<{ user: User; org: Org }> {
  if (process.env.NODE_ENV === "production" || env.BILLING_PROVIDER === "stripe" || !BILLING_ENABLED) notFound();
