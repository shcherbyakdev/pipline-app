...
describe("setupChecklist (admin IA spec §4)", () => {
  ...
  it("both modes: space, service, hours — in that order, all undone", () => {
  ...
  it("appointments-only: no space item", () => {
    expect(setupChecklist({ mode: APPTS, ...nothing }).map((i) => i.id)).toEqual(["service", "hours"]);
  });
  ...
  it("ticks follow the counts", () => {
    const items = setupChecklist({ mode: BOTH, ...nothing, serviceCount: 2, ownersWithHours: 1 });
    expect(Object.fromEntries(items.map((i) => [i.id, i.done]))).toEqual({
      service: true, space: false, hours: true,
    });
  });
