...
describe("reserved list parity with the migration that last defined reserved_handles()", () => {
  it("RESERVED_HANDLES equals the array in reserved_handles()", () => {
    ...
    // migration that redefines the function (0051 first, 0066, 0075, 0076, 0080 since).
    const sql = readFileSync(join(process.cwd(), "src/db/migrations/0080_payments_reserved_handle.sql"), "utf8");
