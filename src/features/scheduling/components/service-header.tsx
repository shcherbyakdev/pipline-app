...
// Text until focused — the dialog's bare-input idiom (space-header.tsx does
// the same for a space's name) — plus a focus ring so a keyboard user can see
// where they are on a page with no borders.
...
/* The service's identity, edited in place: name and description read as
   text and save on blur (Enter commits the name, Escape reverts either).
   Each save sends ONE field — patchService — and snaps back with the
...
export function ServiceHeader({
  ...
  const commit = (field: Field, raw: string) => {
    ...
    startTransition(async () => {
      const result = await patchService({ id: service.id, [field]: value });
